import 'package:avl0401/avl0401.dart' as avl0401;
...
void main() {
  ...
  stmt
    ..execute(['Pedro Farley'])
    ..execute(['Sophia Moura']);
  ...
  final ResultSet resultSet =
      db.select('SELECT * FROM alunos WHERE name LIKE ?', ['%']);

  // You can iterate on the result set in multiple ways to retrieve Row objects
  // one by one.
  for (final Row row in resultSet) {
    print('Alunos[Matricula: ${row['id']}, nome: ${row['name']}]');
  }
